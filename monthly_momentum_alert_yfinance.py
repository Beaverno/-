...
Env vars required: EMAIL_ADDRESS, EMAIL_PASSWORD (optional), EMAIL_RECIPIENT (optional)
"""
...
# -------------------------
# yfinance downloader with check
# -------------------------
...
# 对齐索引
price_df = pd.concat(price_dfs.values(), axis=1)
price_df.columns = list(price_dfs.keys())
...
for i in range(1, n_months):
    ...
    realized_ret = (nav.loc[date]/prev_nav - 1) if prev_nav!=0 else 0.0
...
md, dd_series = (cum_returns.cummax()/cum_returns-1).min(), (cum_returns/cummax()-1)

# Monte Carlo bootstrap
monthly_rets = np.array(portfolio_returns_hist) if portfolio_returns_hist else np.array([0.0])
mc_cagrs, mc_maxdds = [], []
...
for _ in range(MONTE_CARLO_RUNS):
    ...
    mc_maxdds.append((nav_mc/np.maximum.accumulate(nav_mc)-1).min())
...
# trade log df
trade_df = pd.DataFrame(trade_log).sort_values('date') if trade_log else pd.DataFrame()
...
send_email("Momentum Strategy Report (YFinance final)", body_html, attachments=attachments)
